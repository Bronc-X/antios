...
struct GlassButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        ...
        case .danger:
            background = AnyShapeStyle(
                LinearGradient(
                    colors: [Color.statusError, Color(hex: "#FF8D7D")],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
            )
        ...
        return configuration.label
            ...
            .shadow(color: shadowColor, radius: kind == .primary ? 18 : 10, y: kind == .primary ? 10 : 5)
            .brightness(kind == .primary && isPressed ? -0.02 : 0)
            .scaleEffect(isPressed ? 0.976 : 1.0)
