...
struct SettingsView: View {
        ...
        LiquidGlassCard(style: .elevated, padding: 20) {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top) {
                    ...
                    Spacer()
                    StatusPill(
                        text: viewModel.notificationsEnabled ? "已接通" : "待配置",
                        color: viewModel.notificationsEnabled ? .statusSuccess : .statusWarning
                    )
                }
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                    SettingsOverviewMetric(
                        title: "通知",
                        value: viewModel.notificationsEnabled ? "已开启" : "关闭",
                        detail: "低噪触达",
                        tint: .liquidGlassAccent
                    )
                    SettingsOverviewMetric(
                        title: "提醒",
                        value: viewModel.dailyReminderEnabled ? "每日" : "手动",
                        detail: "节奏管理",
                        tint: .liquidGlassWarm
                    )
                    SettingsOverviewMetric(
                        title: "生物识别",
                        value: viewModel.biometricEnabled ? "保护中" : "未启用",
                        detail: "登录安全",
                        tint: .statusSuccess
                    )
                    SettingsOverviewMetric(
                        title: "外观",
                        value: themeManager.appearanceMode.rawValue.capitalized,
                        detail: "界面模式",
    ...
    private var membershipSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("先确认权益和授权链路，再决定要不要打开更深的数据能力。")
        ...
        Button {
            ...
            HStack(spacing: 14) {
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundColor(.statusError.opacity(0.8))
            }
...
private struct SettingsOverviewMetric: View {
    let title: String
    let value: String
    let detail: String
    let tint: Color
