...
struct DigitalTwinView: View {
    ...
    private var header: some View {
        ...
        return LiquidGlassCard(style: .standard, padding: 14) {
            ZStack {
                HStack(spacing: 0) {
                    Button {
                        dismiss()
    ...
    private func summaryStats(_ stats: DigitalTwinCurveSummaryStats) -> some View {
        HStack(spacing: 12) {
            summaryStatCard(title: "整体改善", value: stats.overallImprovement.value, suffix: "分", color: .statusSuccess, icon: "arrow.up.right", prefixPlus: true)
            summaryStatCard(title: "首次见效", value: stats.daysToFirstResult.value, suffix: "天", color: .liquidGlassAccent, icon: "bolt", prefixPlus: false)
            summaryStatCard(title: "坚持度", value: stats.consistencyScore.value, suffix: "%", color: .liquidGlassWarm, icon: "flame", prefixPlus: false)
        }
    }

    private func summaryStatCard(title: String, value: Double?, suffix: String, color: Color, icon: String, prefixPlus: Bool) -> some View {
        LiquidGlassCard(style: .concave, padding: 12) {
            VStack(alignment: .leading, spacing: 8) {
                HStack {
    ...
    private func overviewHero(_ curveData: DigitalTwinCurveOutput) -> some View {
        let dataStatus = curveData.dataQualityStatus()
    ...
    private func trendStatus(
        ...
        if metric.isEmergency(currentValue: currentValue, normalizedDelta: normalizedDelta) {
            return DigitalTwinTrendStatus(
                label: "紧急",
                shadeColor: .statusError
            )
        }
        if normalizedDelta > 4 {
            return DigitalTwinTrendStatus(
                label: "向好",
                shadeColor: .statusSuccess
            )
        }
        return DigitalTwinTrendStatus(
            label: "持平",
            shadeColor: .statusWarning
        )
    }
                ...
                GeometryReader { geo in
                    ...
                    ZStack(alignment: .leading) {
                        Capsule()
                            ...
                            .frame(height: 4)
                        Capsule()
                            .fill(LinearGradient(colors: [.liquidGlassAccent, .liquidGlassFreshGreen], startPoint: .leading, endPoint: .trailing))
    ...
    private func trendMetricPill(_ metric: DigitalTwinTrendMetric) -> some View {
        let isSelected = selectedTrendMetric == metric
        return Button {
            ...
            Text(metric.label)
                ...
                .background(
                    Capsule()
                        .fill(isSelected ? Color.liquidGlassAccent : Color.surfaceGlass(for: colorScheme))
                )
                .overlay(
                    Capsule()
    ...
    private func twinHeroChip(title: String, value: String, tint: Color) -> some View {
