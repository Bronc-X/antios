            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .top) {
                    ...
                    Button {
                        ...
                        .font(.caption.weight(.semibold))
                        .foregroundColor(colorScheme == .dark ? .white : .bgPrimary)
                ...
                HStack(spacing: 10) {
                    FeedHeaderMetric(
                        title: L10n.text("更新节奏", "Cadence", language: language),
                        value: L10n.text("每日", "Daily", language: language)
                    )
                    FeedHeaderMetric(
                        title: "AI",
                        value: L10n.text("已解释", "Annotated", language: language)
                    )
                    FeedHeaderMetric(
                        title: L10n.text("关联", "Relevance", language: language),
                        value: L10n.text("个性化", "Personalized", language: language)
                    )
                }
                ...
                ForEach(ScienceFeedCategory.allCases) { category in
                    Button {
                        ...
                        Text(category.title(language: language))
                            ...
                            .foregroundColor(selectedCategory == category ? (colorScheme == .dark ? .white : .bgPrimary) : .textSecondary)
                            .padding(.horizontal, 14)
                            .padding(.vertical, 10)
                            .background(
...
struct ArticleCard: View {
    ...
    private var cardGradient: LinearGradient {
        LinearGradient(
            colors: [
                ...
                isLight ? Color.brandPaper.opacity(0.92) : Color(hex: "#30284B")
            ],
    ...
    var body: some View {
        ...
        .shadow(color: Color.black.opacity(isLight ? 0.06 : 0.18), radius: 18, y: 10)
    }
...
struct PlatformBadge: View {
    ...
    var body: some View {
        ...
        .overlay(
            Capsule()
                .stroke(Color(hex: platform.color).opacity(0.18), lineWidth: 1)
        )
        .clipShape(Capsule())
    }
}
...
struct MatchBadge: View {
    ...
    private var badgeColor: Color {
        ...
        if pct >= 95 { return .statusSuccess }
        if pct >= 90 { return .statusWarning }
...
private struct FeedHeaderMetric: View {
    let title: String
    let value: String
