...
enum MaxRAGService {
    ...
    private static func mergeMemories(
        ...
        for record in primary + secondary {
            let contentKey = record.renderedContent.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func formatMemories(_ records: [MaxMemoryRecord]) -> String? {
        ...
        let scored = records.map { record -> (MaxMemoryRecord, Double) in
            ...
            switch record.kind {
            case .sensorDerived:
                kindBoost = bodyMemoryBoost
            case .assistantStrategy, .proactiveBrief:
                kindBoost = strategyMemoryBoost
            case .behaviorSignal:
                kindBoost = behaviorMemoryBoost
            case .chatTurn, .generic:
                kindBoost = 0
            }
            let score = similarity * 0.62 + recency * 0.22 + kindBoost
            return (record, score)
        }

        let top = scored.sorted { $0.1 > $1.1 }.prefix(memoryBlockTopN).map { $0.0 }
        let lines = top.map { record in
            let roleTag = record.kind.ragTag(for: record.role)
            return "• [\(roleTag)] \(record.renderedContent)"
        }
    ...
    private static var bodyMemoryBoost: Double {
        runtimeDouble(for: "MAX_RAG_BODY_MEMORY_BOOST", fallback: 0.24, min: 0.0, max: 0.5)
    }

    private static var behaviorMemoryBoost: Double {
        runtimeDouble(for: "MAX_RAG_BEHAVIOR_MEMORY_BOOST", fallback: 0.06, min: 0.0, max: 0.3)
    }

    private static var strategyMemoryBoost: Double {
        runtimeDouble(for: "MAX_RAG_STRATEGY_MEMORY_BOOST", fallback: 0.1, min: 0.0, max: 0.3)
    }
}
